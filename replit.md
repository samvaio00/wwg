...
- **Technical Implementations & Features**:
    ...
    - **Image Storage**: Persistent local storage for Zoho product images in `public/product-images/`. Images are downloaded once from Zoho and served directly from disk, eliminating API rate limiting issues. Admin endpoints: `/api/admin/images/sync` for bulk download, `/api/admin/products/:id/refresh-image` for single image refresh. Features image fallback logic: tries item-level image first, then falls back to group-level image for grouped products. Webhook-triggered background image pulls automatically download images when products are created/updated via Zoho webhooks. **Image source tracking**: Products and groups have an `imageSource` field ('uploaded' or 'zoho') that protects manually uploaded images from being deleted or overwritten by ANY Zoho sync or refresh, including admin force refresh. **Website-uploaded images ALWAYS take precedence** over Zoho images - no automatic or manual Zoho refresh/sync will overwrite them. To change a website-uploaded image, admin/staff must upload a new one directly. **Permanent image storage**: Once an image exists locally (from any source), it is permanent - Zoho syncs only check for images when none exists, and images are only replaced when admin/staff explicitly uploads a new one.
    ...
    - **Job Queue & Retries**: Manages retryable Zoho operations for robustness. Features exponential backoff retry mechanism for Zoho API rate limiting (30s → 60s → 120s → 240s → 300s max), applied to both Zoho Inventory and Zoho Books services.
