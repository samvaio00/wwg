...
- **Technical Implementations & Features**:
    ...
    - **Scheduler**: Customer status sync (hourly), embeddings updates (2 hours), weekly top sellers sync, bi-weekly AI email campaigns, and weekly Zoho backup sync (Sunday 2 AM). Frequent API polling is disabled by default (dormant) in favor of webhooks.
    - **Zoho Webhooks**: Real-time product and customer updates from Zoho Inventory and Zoho Books via webhooks, with security through a shared secret. Webhook stats are tracked and displayed in admin dashboard.
